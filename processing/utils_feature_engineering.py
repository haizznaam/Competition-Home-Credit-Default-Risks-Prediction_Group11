...
def feature_evaluate(pdf_train, pdf_feat, ls_feat=None):
    ...
    pdf_res = pdf_res[["name", "auc", "corr", "coverage"]
    ...
    for k in dict_feat:
        if k == "binary_default":
            for cname in dict_feat[k]:
                ...
                pdf_data[feat_name] = pdf_data[cname].apply(
                    lambda x: int(x == default_val))
        elif k == "binary":
            # rename only
            for cname in dict_feat[k]:
                feat_name = "is_" + cname
            ...
            for cname in dict_feat[k]:
                ...
                for val in ls_vals:
                    try:
                        ...
                        pdf_data[new_name] = pdf_data[cname].apply(
                            lambda x: int(x == val))
...
def agg_common_data(pdf_input, ls_func, main_key="SK_ID_CURR"):
    ...
    rename_cols = ["{}_{}".format(tpl[0], tpl[1])
                   for tpl in zip(name01, name02)]
...
def agg_mean_encoding(pdf_input, ls_cat):
    ...
    for col_to_encode in ls_cat:
        ...
        pdf_mean_encoding, s_mean_encoding = mean_encoding(
            pdf_input, col_to_encode)
...
def mean_encode_mapping(pdf_input, dict_encoding_map):
    ...
    ls_encode_col = list(dict_encoding_map.keys())
